...
export default class InviteCommand extends Command {
  ...
  public execute(interaction: CommandInteraction<CacheType>): Promise<void> {
    const row = new MessageActionRow()
          ...
          .setURL(`https://discord.com/oauth2/authorize?client_id=${interaction.client.user.id}&permissions=268725328&scope=bot`),
			);
    return interaction.reply({ content: `Add **${interaction.client.user.username}** to a server by clicking the following button:`, components: [row] });
    }
