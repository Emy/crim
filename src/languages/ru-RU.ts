  ...
  constructor(client: KlasaClient, store: LanguageStore, file: string[], directory: string, options?: LanguageOptions) {
    ...
    this.language = {
      ...
      RESOLVER_INVALID_EMOJI: (name) =>
        `${name} должен быть тегом пользовательского эмодзи или действительным id эмодзи.`,
      ...
      RESOLVER_INVALID_REGEX_MATCH: (name, pattern) =>
        `${name} должен соответствовать регулярному выражению \`${pattern}\`.`,
      ...
      RESOLVER_MINMAX_EXACTLY: (name, min, suffix) =>
        `${name} должен быть равен ${min}${suffix === this.language.RESOLVER_STRING_SUFFIX ? `${suffix}ам` : suffix}.`,
      RESOLVER_MINMAX_BOTH: (name, min, max, suffix) =>
        `${name} должен быть между ${min} и ${max}${
          suffix === this.language.RESOLVER_STRING_SUFFIX ? `${suffix}ами` : suffix
        }.`,
      RESOLVER_MINMAX_MIN: (name, min, suffix) =>
        `${name} должен быть больше ${min}${suffix === this.language.RESOLVER_STRING_SUFFIX ? `${suffix}ов` : suffix}.`,
      RESOLVER_MINMAX_MAX: (name, max, suffix) =>
        `${name} должен быть меньше ${max}${suffix === this.language.RESOLVER_STRING_SUFFIX ? `${suffix}ов` : suffix}.`,
      ...
      MONITOR_COMMAND_HANDLER_REPROMPT: (tag, error, time, abortOptions) =>
        `${tag} | **${error}** | Ответьте на это сообщение допустимым значением аргумента в течение **${time}** ${this.pluralize(
          time,
          'секунды',
          'секунд',
          'секунд',
        )} или отправьте **${abortOptions.join('**, **')}** для отмены.`,
      MONITOR_COMMAND_HANDLER_REPEATING_REPROMPT: (tag, name, time, cancelOptions) =>
        `${tag} | **${name}** является повторяющимся аргументом | Ответьте на это сообщение дополнительными значениями аргумента в течение **${time}** ${this.pluralize(
          time,
          'секунды',
          'секунд',
          'секунд',
        )} или отправьте **${cancelOptions.join('**, **')}** для отмены.`,
      MONITOR_COMMAND_HANDLER_ABORTED: 'Отменено',
      INHIBITOR_COOLDOWN: (remaining) =>
        `Вы только что использовали эту команду. Вы сможете использовать её снова через ${remaining} ${this.pluralize(
          remaining,
          'секунды',
          'секунд',
          'секунд',
      ...
      INHIBITOR_MISSING_BOT_PERMS: (missing) =>
        `Недостаточно прав для выполнения команды, отсутствуют следующие разрешения: **${missing}**`,
      ...
      INHIBITOR_REQUIRED_SETTINGS: (settings) =>
        `Команда не может быть выполнена, так как на сервере отсутствуют следующие настройки: **${settings.join(
          ', ',
        )}**.`,
      ...
      COMMAND_EVAL_SENDFILE: (time, type) =>
        `Результат оказался слишком длинным, он будет отправлен в виде файла.\n**Тип**:${type}\n${time}`,
      COMMAND_EVAL_SENDCONSOLE: (time, type) =>
        `Результат оказался слишком длинным, он будет выведен в консоль.\n**Тип**:${type}\n${time}`,
      COMMAND_UNLOAD: (type, name) => `✅ Выгружено ${type}: ${name}`,
      COMMAND_UNLOAD_DESCRIPTION: 'Выгружает элемент klasa.',
      COMMAND_UNLOAD_WARN:
        'Скорее всего вам не стоит выгружать это, так как Вы не сможете использовать какую-либо команду, чтобы загрузить это снова.',
      ...
      COMMAND_TRANSFER_FAILED: (type, name) =>
        `Перемещение ${type}: ${name} в клиент не удалось, проверьте вашу консоль.`,
      ...
      COMMAND_LOAD_ERROR: (type, name, error) =>
        `❌ Загрузка не удалась ${type}: ${name}. Причина:${util.codeBlock('js', error)}`,
      ...
      COMMAND_ENABLE_DESCRIPTION:
        'Активирует или временно активирует команду/ингибитор/монитор/финализатор. Состояние по умолчанию будет восстановлено после перезагрузки.',
      COMMAND_DISABLE: (type, name) => `+ Успешно деактивировано ${type}: ${name}`,
      COMMAND_DISABLE_DESCRIPTION:
        'Деактивирует или временно деактивирует команду/ингибитор/монитор/финализатор/событие. Состояние по умолчанию будет восстановлено после перезагрузки.',
      COMMAND_DISABLE_WARN:
        'Скорее всего вам не стоит деактивировать это, так как Вы не сможете использовать какую-либо команду, чтобы активировать это снова.',
      ...
      PLAY: `Проигрывание`,
      ADDED_TRACK: (track) => `**${track}** добавлен в очередь!`,
      ADDED_PLAYLIST: (playlist) => `**${playlist}** добавлен в очередь!`,
      QUEUE: `Очередь`,
      SKIP: `Пропуск`,
      ...
      PAUSE_DESCRIPTION: 'Останавливает / возобновляет воспроизведение.',
      ...
      STOP_DESCRIPTION: 'Останавливает текущую сессию и очищает очередь.',
