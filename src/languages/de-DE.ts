  ...
  constructor(client: KlasaClient, store: LanguageStore, file: string[], directory: string, options?: LanguageOptions) {
    ...
    this.language = {
      ...
      SETTING_GATEWAY_VALUE_FOR_KEY_NOEXT: (data, key) =>
        `Für das Attribut ${key} ist der Wert ${data} nicht vorhanden.`,
      SETTING_GATEWAY_VALUE_FOR_KEY_ALREXT: (data, key) =>
        `Für das Attribut ${key} ist der Wert ${data} bereits vorhanden.`,
      ...
      COMMANDMESSAGE_NOMATCH: (possibles) =>
        `Deine Auswahl trifft auf keine der folgenden Möglichkeiten zu: (${possibles})`,
      MONITOR_COMMAND_HANDLER_REPROMPT: (tag, error, time) =>
        `${tag} | **${error}** | Sie haben **${time}** Sekunden, um einen gültigen Wert einzugeben. Mit "ABORT" brechen Sie den Vorgang ab.`,
      MONITOR_COMMAND_HANDLER_ABORTED: 'Abgebrochen',
      INHIBITOR_COOLDOWN: (remaining) =>
        `Sie haben gerade erst diesen Befehl benutzt. Der Befehl kann von Ihnen wieder in ${remaining} Sekunden verwendet werden.`,
      ...
      INHIBITOR_REQUIRED_SETTINGS: (settings) =>
        `Aufgrund der fehlenden Einstellung **${settings.join(', ')}** kann dieser Befehl nicht ausgeführt werden.`,
      INHIBITOR_RUNIN: (types) => `Dieser Befehl kann nur in ${types} Kanälen ausgeführt werden`,
      INHIBITOR_RUNIN_NONE: (name) =>
        `Der ${name} Befehl wurde nicht für die Benutzung in mindestens einem Kanal aktiviert.`,
      ...
      COMMAND_TRANSFER_FAILED: (type, name) =>
        `Übertrag von ${type}: ${name} zum Client fehlgeschlagen. Bitte überprüfe deine Konsole.`,
      ...
      COMMAND_HELP_NODM:
        '❌ | Ich konnte Ihnen keine Direktnachricht schicken, da Sie den Erhalt von Direktnachrichten deaktiviert haben.',
      ...
      COMMAND_DISABLE_WARN:
        'Sie sollten diesen Befehl nicht deaktivieren, da Sie danach nicht mehr befähigt wären, irgendeinen Befehl wieder zu aktivieren.',
      ...
      COMMAND_CONF_KEY_NOT_ARRAY:
        'Dieses Attribut ist nicht vom Typ "array". Bitte benutzen Sie stattdessen die Aktion \'reset\'.',
      ...
      COMMAND_ENABLE_DESCRIPTION:
        'Wiederaktiviert oder zeitweise aktiviert ein Befehl/Inhibitor/Monitor/Finalizer. Wird bei einem reboot auf Standard zurückgesetzt.',
      COMMAND_DISABLE_DESCRIPTION:
        'Deaktiviert oder zeitweise deaktiviert ein Befehl/Inhibitor/Monitor/Finalizer. Wird bei einem reboot auf Standard zurückgesetzt.',
      ...
      UNKNOWN_ERROR:
        'Unbekannter Fahler aufgetreten! Wenn dieses Problem weiterhin besteht kontaktire bitte meinen Meister!',
