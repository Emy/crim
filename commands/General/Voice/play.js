...
module.exports = class extends Command {
  ...
  async run(message, [song]) {
    if (!message.member.voice.channel) return message.sendError('ERROR_NOT_IN_VC');
    const data = await fetch(`http://${nodes[0].host}:${nodes[0].port}/loadtracks?identifier=ytsearch:${song}`, {
      headers: {
